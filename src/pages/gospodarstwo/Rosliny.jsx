  ...
  const handleDelete = (magazynId) => {
    fetch(`/api/magazyn/${magazynId}`, {
      ...
      .then((data) => {
        ...
        setRosliny(rosliny.filter((roslina) => roslina.magazyn_id !== magazynId));
      })
      .catch((error) => {
        console.error("Error deleting roslina:", error);
        // Handle error
      });
  };
  return (
      ...
      <div className="align-middle inline-block min-w-full shadow overflow-hidden bg-white shadow-dashboard rounded">
        <table className="min-w-full">
          ...
          <tbody className={isScrollable ? "scrollable-table-magazyn" : ""}>
            {rosliny.map((roslina, index) => (
              <tr key={index} className={index % 2 === 0 ? "bg-gray-100" : "bg-white"}>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-r border-gray-500 text-sm leading-5 text-center align-middle w-72">{roslina.nazwa}</td>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-r border-gray-500 text-sm leading-5 text-center align-middle w-40">{roslina.data_zakupu}</td>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-r border-gray-500 text-sm leading-5 text-center align-middle w-36">
                  {roslina.cena_netto} zł
                </td>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-r border-gray-500 text-sm leading-5 text-center align-middle w-36">
                  {roslina.cena_brutto} zł
                </td>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-r border-gray-500 text-sm leading-5 text-center align-middle w-36">{roslina.ilosc}</td>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-r border-gray-500 text-sm leading-5 text-center align-middle w-36">
                  {roslina.jednostka_magazynowa}
                </td>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-r border-gray-500 text-sm leading-5 text-center align-middle w-40">{roslina.wartosc}</td>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-r border-gray-500 text-sm leading-5 text-center align-middle w-48">
                  {roslina.zalecana_dawka}
                </td>
                <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-500 text-sm leading-5 text-right align-middle w-36">
                  <Link to={`/edytuj-magazyn/${roslina.magazyn_id}`} className="text-indigo-600 hover:text-indigo-900 px-2">
                    <button>Edytuj</button>
                  </Link>
                  <button onClick={() => handleDelete(roslina.magazyn_id)} className="text-red-600 hover:text-red-900 mx-auto">
                    Usuń
                  </button>
                </td>
              </tr>
